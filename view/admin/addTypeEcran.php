...
<html>
    <head>
        <title>Admin-Add-TypeEcran</title>
        <meta charset="UTF-8">
        ...
        <div id="all_admin_page">
            <?php
                if (isset($params['add']) && !empty($params['add']) && $params['add'] == 2) {
                    echo '<div id="erreur">';
                    echo '<p>Un problème est survenu pendant la création du type ecran</p>';
                    echo '</div>';
                }
            ?>
            <form id="idForm" action="admin/addTypeEcran/" method="post">
                <div>
                    <p>Libelle type ecran :</p>
                    <input type="text" name="libelle" <?php if (isset($params['libelle']) && isset($params['add']) && $params['add'] == 2) {echo 'value ="'.$params['libelle'].'"';}?>>
                </div>
                <div>
                    <input id="btSubmit" type="submit" name="formTypeEcran" value="Ajouter">
                </div>
            </form>
        </div>
